...
/*
  Recommends not to use any of the builtin thread pools
...
public class R10_SubscribeObserveOn {
	...
	// acts like a 1.8 completeable future
	...
	public void observeOn() throws Exception {
		slowFromCallable()
				.subscribeOn(Schedulers.io())
				.doOnNext(x -> log.info("A: {}", x))  // doOnNext is useful for debug
				.observeOn(Schedulers.computation())  // observeOn allows a switch to a new thread
  ...
  final ThreadFactory threadFactory = new ThreadFactoryBuilder()
          .setNameFormat("CustomerExecutor-%d+")
          .build();
